...
result still rebuilds both maps and copies each nested `PluginConfig` and
`ServiceSettings` value. Mutating the copy, including nested `config` maps,
...
is resolved with `??`. To clear an override, call
`withClearedPriority()` on the `ServiceSettings` instance.
</Aside>
...
| `list<T>(key)` | `List<T>?` | Returns an unmodifiable `List<T>` copy when the value is a `List` and the cast succeeds. For `T == Map<String, dynamic>`, each element is copied as `Map<String, dynamic>.from(...)`. Returns `null` on cast failure or non-list value. |
| `map(key)` | `Map<String, dynamic>?` | Returns `Map<String, dynamic>.from(value.cast<String, dynamic>())` when the value is a map and the cast succeeds. Returns `null` on cast failure or non-map value. |

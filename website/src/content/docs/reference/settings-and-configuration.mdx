...
```dart
const mainAgent = PluginId('main_agent');
const agent = Namespace('agent');

final settings = RuntimeSettings(
  services: {
    // Plugin-scoped: main_agent's agent.temperature service.
    mainAgent.namespace('agent').service('temperature'):
        const ServiceSettings(config: {'value': 0.7}),

    // Equivalently, hand-built Pin:
    Pin('main_agent', ['agent', 'temperature']):
        const ServiceSettings(config: {'value': 0.7}),

    // Wildcard: applies to whichever plugin wins agent.system_message.
    Pin.wildcard(['agent', 'system_message']):
        const ServiceSettings(config: {'value': 'Be concise.'}),
  },
);
```
...
```dart
final pin = Pin.fromWire('main_agent:agent.temperature');
...
```dart
final headers = config.map('headers');
...
```dart
final temperature = config.getDouble('temperature') ?? 0.7;
final tools = config.list<String>('tools') ?? const [];
final provider = config.getString('provider') ?? 'openai';
```
...
<CardGrid>
  ...
  <LinkCard
    ...
    description="PluginRuntimeManager.updateSettings and the reconciliation phases that consume these types."

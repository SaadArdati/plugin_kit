...
Plugin enablement falls back through three rules: an explicit `RuntimeSettings.plugins` entry wins, then `FeatureFlag.locked` plugins are always on, then `FeatureFlag.experimental` plugins default off while all other plugins default on. Build a `RuntimeSettings` listing the subset you want enabled and pass it to `init` (or to `updateSettings` later).

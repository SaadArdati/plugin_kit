...
| `onPluginSettingsChanged` | During `updateGlobalSettings` or `updateSessionSettings` for every plugin enabled after the update (including newly enabled plugins) | Receives `oldContext` and `newContext` so the plugin can diff and react. |

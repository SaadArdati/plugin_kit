...
- **Lazy initialization.** Work runs only when something asks for it. Toggling a plugin off and on does not re-run startup code; it just removes and re-adds its services.

`Plugin.attach` and `Plugin.detach` are pure user hooks: the framework wraps them with orchestration (context binding, owned `StatefulPluginService` attach/detach, subscription cleanup), so there is no `super.attach` / `super.detach` to call. The one hook that does require `super` is `PluginService.injectSettings(...)`. See [Plugin Services](/concepts/plugin-services/).

...
import runtimeTestSession from '../../../../../packages/plugin_kit/test/runtime_test.dart?raw';
import themeServiceBroadcast from '../../../../snippets/lib/sessions.dart?raw';
...
<Code code={extractRegion(runtimeTestSession, 'runtime-test-session')} lang="dart" />
...
<Code code={extractRegion(themeServiceBroadcast, 'theme-service-broadcast')} lang="dart" />
...
<Code code={extractRegion(themeServiceBroadcast, 'sessions-emit-global-bus')} lang="dart" />

### Global plugin to global plugin

Uses the global bus directly:

<Code code={extractRegion(themeServiceBroadcast, 'sessions-emit-global-plugin')} lang="dart" />

### Session plugin to session plugin (same session)

Uses that session's bus:

<Code code={extractRegion(themeServiceBroadcast, 'sessions-emit-session-plugin')} lang="dart" />

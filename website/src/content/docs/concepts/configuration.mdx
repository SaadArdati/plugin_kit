...
| `map(key)` | `Map<String, dynamic>` lookup, or `{}` when absent or invalid |
...
`map(...)` returns an empty map when the value is missing or cannot be cast. You never end up with a null reference exception from this API.
...
- Override `injectSettings(...)` in the service for service-local reactions.

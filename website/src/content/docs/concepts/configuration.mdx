...
```dart
class ModelConfig extends PluginService {
  String get provider => config.getString('provider') ?? 'openai';
  String get model => config.getString('model') ?? 'gpt-4.1-mini';
  double get temperature => config.getDouble('temperature') ?? 0.7;
  bool get streaming => config.getBool('streaming') ?? true;
  List<String> get tools => config.list<String>('tools') ?? const [];
}
```
...
final settings = RuntimeSettings(
  services: {
    Pin('router', ['model_config']): const ServiceSettings(
      config: {
        'provider': 'anthropic',
...
final timeoutMs = headers['timeout_ms'];
```

...
class CeoLovesThisCapability extends Capability {
  const CeoLovesThisCapability();
}
```
...
registry.registerSingleton<FormatterService>(
  const ServiceId('formatter'),
  SqlFormatter(),
  capabilities: {
    const FormatterCapability(['sql']),
    const IsSlowCapability(isSlow: false),
  },
);
```

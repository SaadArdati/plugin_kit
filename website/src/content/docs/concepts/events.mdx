...
## Subscribing from a plugin (escape hatch, not the recommended path)
...
by hand: the bus call returns a `StreamSubscription` you have to cancel yourself, and is not tracked.

<Aside type="caution" title="Behavior on the plugin gives up composition">
...
Plugin-level subscriptions are appropriate for trivial wiring (one debug log, a tracing tap, a small
bridge between two services the same plugin owns). For anything you want to be a real seam, write a
service. If a plugin's `attach` starts accumulating handlers, that is a smell: extract a service.
</Aside>

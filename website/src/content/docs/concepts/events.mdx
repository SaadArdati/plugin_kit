...
```dart
class ChatLogger extends StatefulPluginService {
  @override
  void attach() {
    on<UserMessageReceived>((e) {
      log('chat', '${e.event.sessionId}: ${e.event.text}');
    });

    on<OutgoingReply>((e) {
      if (e.event.isDraft) {
        e.stop(const ReplyDiscarded());
      }
    });
  }
}
...
// A plugin that expands typed macros like /shrug before the message goes out.
class MacroExpander extends StatefulPluginService {
  ...
  @override
  void attach() {
    on<DraftOutgoingMessage>((e) {
      final msg = e.event;
      _macros.forEach((macro, replacement) {
        msg.text = msg.text.replaceAll(macro, replacement);
      });
      msg.metadata['macros_expanded'] = 'true';
    });
  }
}

// The emitter acts on whatever the chain produced.
final envelope = await context.bus.emit<DraftOutgoingMessage>(
  event: DraftOutgoingMessage(userInput),
);

if (envelope.stopped) {
  // A handler vetoed or replaced the message entirely.
  return;
}

await sendToServer(envelope.event); // the transformed payload
```
...
class PortAllocator extends StatefulPluginService {
  final _ports = <int>{};

  @override
  void attach() {
    onRequest<FindOpenPort, int?>((e) async {
      for (var port = 8000; port < 9000; port++) {
        if (_ports.add(port)) return port;
      }
      return null; // concede; let the next handler try
    });
  }
}
```

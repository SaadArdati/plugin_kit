...
the cascade and pin the final result. Lower-priority handlers only run if no earlier one stopped. Default is `Priority.normal`; same convention and same default as the [service registry](/concepts/service-registry/#priority).

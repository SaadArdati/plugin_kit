...
| `StatefulPluginService<Ctx>` | the service needs `attach()` / `detach()`, a bound session context, auto-tracked subscriptions, and a place for session-bound state |

If your plugin class is getting fat, that is almost always a hint that the work belongs in one of these three tiers, not on the plugin. Plain classes register as factories (fresh instance per `context.resolve<T>()`) or as singletons when sharing the instance is safe. `PluginService` and `StatefulPluginService` register as singletons or lazy singletons; the stateful variant rejects factories because the runtime needs an instance to track.

This page is the natural next step after [Adding a Plugin](/guides/adding-a-plugin/): the plugin class stays focused on wiring, services carry settings, state, and long-lived behavior.
...
`StatefulPluginService` extends `PluginService` with session lifecycle. It adds `attach(context)`, `detach()`, a bound `context`, and automatic subscription tracking.

<Code code={extractRegion(pluginServicesSnippets, 'stateful-plugin-service-basic')} lang="dart" />

`attach()` and `detach()` are pure user hooks: no super call, no `context` parameter. The framework binds and unbinds `this.context` around them via library-private `_bindContext` / `_unbindContext`, and cancels every tracked subscription and binding after `detach()` returns. Helpers (`on`, `onRequest`, `bind`, `emit`, `resolve`, etc.) read `this.context` implicitly, so subscriptions inside `attach()` are always bucketed correctly.

The one method that does require super is `injectSettings(...)`, which carries the actual settings injection logic. See "Reacting to settings changes" below.

## Reacting to settings changes

`PluginService.injectSettings(...)` is the handoff point for effective config. The base implementation
stores the raw settings, builds a fresh `ConfigNode`, and updates the cached hash.

If your service needs to react when settings change (to throw away a compiled cache, 
for example), override it and call `super`.

<Code code={extractRegion(pluginServicesSnippets, 'stateful-plugin-service-inject-settings')} lang="dart" />

For singletons and lazy singletons, the registry hashes effective settings and
only re-injects when they actually changed. Each individual `config.getX(...)` read is O(1): a hash-map
lookup plus at most one type check. The O(n) cost (a deep hash over the new settings) only
pays out when `injectSettings` actually runs again.
...
The service-level helpers are thin wrappers over the session context:
...
- timers
- session-bound caches
- mutable session state

If you find yourself writing `final _subs = <StreamSubscription>[];` on a `SessionPlugin`, that is usually the signal to promote the state to a stateful service.
...
- `StatefulPluginService`: session-bound behavior and subscriptions.

...
If your app already uses [`hyper_logger`](https://github.com/codelessly/hyper_logger), Plugin Kit records flow through its root subscription automatically. No extra setup.
...
try {
  ...
  print('${e.failures.length} plugin(s) failed during ${e.phase}:');
  for (final (pluginId, error, stackTrace) in e.failures) {
    print('  $pluginId: $error');
  }
}
```

...
manager class that slowly became the local government. Plugin Kit is not asking
...
[State Management Integrations](/guides/state-management-integrations/). This
...
```dart
class AlgoliaSearchPlugin extends SessionPlugin {
  @override
  PluginId get pluginId => const PluginId('search.algolia');

  @override
  void register(ScopedServiceRegistry registry) {
    registry.registerLazySingleton<SearchService>(
...
class AlgoliaSearchPlugin extends SessionPlugin {
  ...
  void attach(SessionPluginContext context) {
    onRequest<SearchRequested, SearchResult?>(context, (request) async {
      final results = await performSearch(request.event.query);
      return SearchResult(results);
    });
  }
}

final result = await session.maybeRequest<SearchRequested, SearchResult?>(
  const SearchRequested('buttons'),
);
```
...
| a chat manager calling "gather context", then "pick model", then "send", then "stream progress" | one outgoing action plus typed events around it | plugins can join the turn without the screen owning every phase |
...
class _EditorScreenState extends State<EditorScreen> {
  late final PluginRuntimeManager _plugins;
...
```dart
class FileSaveRequested {
  final String path;
  final String contents;
  const FileSaveRequested({required this.path, required this.contents});
}
...
class SendMessageRequested {
  ...
  const SendMessageRequested(this.prompt);
}

class ChatBridge extends ChangeNotifier {
  ...
  ChatBridge(this.session) {
    _messagesSub = session.on<ChatMessagesChanged>((envelope) {
      if (_disposed) return;
      messages = envelope.event.messages;
      notifyListeners();
    });
  }
...
```dart
final bridge = context.watch<ChatBridge>();
...
```dart
final pluginManagerProvider = Provider<PluginRuntimeManager>((ref) {
...
class ChatCubit extends Cubit<ChatState> {
  ...
  ChatCubit(this.session) : super(ChatState.initial()) {
    _messagesSub = session.on<ChatMessagesChanged>((envelope) {
      if (isClosed) return;
...
That said, the Cubit-as-bridge is the most redundant of the integrations on the [State Management Integrations](/guides/state-management-integrations/) page. Read `_messagesSub`'s callback: it takes a bus event and re-emits it as widget state. The bus already shipped that event; the Cubit is forwarding it. Keep the Cubit if `state.copyWith` ergonomics or `BlocBuilder` rebuild filtering pay for themselves; reach past it if not.
...
class AssistantRuntimeService extends StatefulPluginService {
  ...
  void attach() {
    Future(() async {
      ...
      emit(AssistantReady(assistant));
    });
  }
}

// Anywhere that needs the assistant subscribes once and reacts:
on<AssistantReady>((envelope) {
  // Use envelope.event.assistant.
});
```
...
final assistant = await session.maybeRequest<WaitForAssistant, AssistantClient?>(
  const WaitForAssistant(),
);

if (assistant == null) {
  throw StateError('Assistant is not available.');
}
```
...
```dart
class MessageChunkReceived {
  final String text;
  const MessageChunkReceived(this.text);
}
...
Future<void> streamAssistantResponse(
  ...
  await for (final chunk in assistant.stream(prompt)) {
    await session.emit(MessageChunkReceived(chunk.text));
  }

  await session.emit(const MessageCompleted());
}
```
...
```dart
class LegacySearchAdapterPlugin extends SessionPlugin {
  @override
  PluginId get pluginId => const PluginId('legacy_search_adapter');

  @override
  void attach(SessionPluginContext context) {
    onRequest<SearchRequested, SearchResult?>(context, (request) async {
      final legacy = GetIt.I<SearchService>();
      final hits = await legacy.query(request.event.query);
...
something else participate here without hard-coding it into this manager?"
...
2. Add a `PluginRuntimeManager` at the app, workspace, or route boundary.

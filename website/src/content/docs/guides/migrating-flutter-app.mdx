...
If events cross a server boundary, make them serializable. For requests,
handlers run in descending priority order (highest first); a non-null return
claims the call and stops dispatch, while returning `null` concedes to the next
handler. That concession-based cascade works even when the response type is
non-nullable.

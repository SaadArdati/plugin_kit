...
| `chat_manager` | Two words, snake_case |
...
| `chatManager` | camelCase reads inconsistently next to the snake_case rest of the runtime |
| `chat-manager` | Hyphens are not the convention; the entire codebase uses underscores |
...
class DocumentSaved {
  ...
  const DocumentSaved(this.documentId);
}
```
...
class IsSlowCapability extends Capability {
  final bool isSlow;
  const IsSlowCapability({this.isSlow = true});
}
```
...
const settings = RuntimeSettings(
  plugins: {
    PluginId('analytics'): PluginConfig(enabled: false),
    PluginId('experimental_router'): PluginConfig(enabled: true),
  },
);
```
...
final settings = RuntimeSettings(
  services: {
    // Plugin-scoped: one specific plugin's registration.
    linter.service(const ServiceId('line_length_linter')):
        const ServiceSettings(config: {'max_line_length': 120}),

    // Equivalent hand-built Pin:
    Pin('linter_suite', ['line_length_linter']):
        const ServiceSettings(config: {'max_line_length': 120}),

    // Wildcard: applies to whichever plugin wins agent.model.
    Pin.wildcard(['agent', 'model']):
...
| Variable | Refers to |
|---|---|
| `manager` | A `PluginRuntimeManager` |
| `runtime` | A `PluginRuntime`, often via `manager.runtime` |

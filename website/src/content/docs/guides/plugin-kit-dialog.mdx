...
Each resolved winner in `runtime.sessions.lastOrNull?.registry ?? runtime.globalRegistry` that ships a `UiConfigurableCapability` becomes an editable card. Text, numbers, dropdowns, switches, multiline, password, and grouped fields are supported.

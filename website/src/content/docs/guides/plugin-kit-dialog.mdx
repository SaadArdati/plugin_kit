...
The dialog is wholly dogfooded: it builds itself out of `plugin_kit` plugins, so every tab, header action, and field renderer is a real plugin you can shadow or replace from your host app.
...
`onSave` is awaited before the dialog closes, so you can persist (write to disk, push to a server, call `manager.updateSettings(...)`) without racing the dismissal.
...
Because the visuals plugin lives in your host app (which has Flutter), Dart-only plugins still get rich visuals without importing Flutter. The decoration is keyed by `PluginId`, `Namespace`, or `ServiceId`, so the host owns the map and the plugin source code stays portable.
...
`onSave` runs before the dialog closes. If you only persist (write to disk), the runtime stays on its old settings until something calls `manager.updateSettings(...)`. If you want the runtime to converge live as the user clicks Save, call `await manager.updateSettings(settings)` from `onSave`. See [Settings & Overrides](/guides/settings/) for the reconciliation model.
</Aside>

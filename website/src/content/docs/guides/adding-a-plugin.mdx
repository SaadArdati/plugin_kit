...
<Code code={extractRegion(pluginServicesSource, 'adding-plugin-notification-plugin')} lang="dart" />
...
<Code code={extractRegion(pluginServicesSource, 'adding-plugin-notification-reactive')} lang="dart" />

The plugin class itself did not override `attach(...)`. When the session
starts, the framework walks every registered, non-disabled
`StatefulPluginService` (factories skipped) the plugin owns and attaches
each one before invoking the plugin's own

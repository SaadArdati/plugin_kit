...
**Dispose the runtime, not individual sessions.** `PluginRuntime.dispose()` iterates active sessions and disposes them. Calling `session.dispose()` and `runtime.dispose()` separately races on stateful service detach and can throw `ConcurrentModificationError`. One call. The runtime.
...
Resist the urge to stuff `PluginRuntime` into an `InheritedWidget` or a Provider that rebuilds the runtime whenever the tree rebuilds. The runtime outlives the widget tree (hot restart, deep navigation, route stack resets all throw away widgets but not the runtime). Hold it somewhere whose lifetime matches the runtime's: a top-level `final`, a GetIt singleton, a Riverpod provider with one-shot create, or, if the scope's lifetime really does match the runtime's, `flutter_plugin_kit`'s `PluginRuntimeScope`, which constructs the runtime in `initState` and disposes it in `dispose` without rebuilding it.
</Aside>

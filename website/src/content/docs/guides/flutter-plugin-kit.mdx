...
- `listen<E>(void Function(EventEnvelope<E> envelope) handler)`: register a handler for events of type `E`. Returns nothing; cancellation is automatic on `dispose`. The handler receives the full envelope so `envelope.sourcePluginId` / `envelope.timestamp` stay reachable; read the payload via `envelope.event`.
- `rebuildOn<E>([bool Function(EventEnvelope<E> envelope)? when])`: register a setState-trigger handler for events of type `E`, optionally filtered by `when`. The predicate receives the envelope too, so filtering can gate on metadata.
...
`watchEvent<E>` reads the ambient `PluginSessionScope`, subscribes the calling element to events of type `E`, and rebuilds the element when one arrives. Returns the latest `E?` (initially `null`). The subscription is cleaned up when the element is unmounted, with no manual cancel needed.
...
Same shape. Subscribe in a notifier or store, expose the latest event, dispose cancels. Each library's recipe lives in [`example/state_garden/`](https://github.com/SaadArdati/plugin_kit/tree/main/example/state_garden) and is shown side-by-side in [State Management Bridges](/reference/state-management-bridges/).

...
- `listen<E>(void Function(EventEnvelope<E> envelope) handler, {int priority = 0, String? identifier})`: register a handler for events of type `E`. Returns nothing; cancellation is automatic on `dispose`. The handler receives the envelope, so `envelope.identifier`, `envelope.event`, and `envelope.stopped` stay reachable.
...
- You need `priority` or `identifier` on the subscription. `watchEvent` registers as a no-identifier handler at `EventBus.on`'s default priority (`Priority.normal`). `PluginEventNotifier` forwards both to `EventBus.on`, but its constructor default `priority` is `0`, so pass `Priority.normal` if you want the bus default.

<Code code={extractRegion(flutterIntegrationSource, 'flutter-plugin-kit-provider-notifier')} lang="dart" />

...
<Aside type="caution" title="In-process plugins only (today)">
Every plugin compiles into your application binary. There is no runtime mechanism to load plugins from a separate process, a network endpoint, or a sandboxed worker. Out-of-process plugin loading (over IPC, WebSocket, or a wasm boundary) is **planned for a future release** but not yet implemented. If you need cross-process plugins today, this runtime is not yet that, but it is being built toward it.
</Aside>
...
Plugin Kit Dialog, the customization surface that ships with the library. Toggling a plugin updates the working draft, and saving applies lifecycle changes. The runtime is fully introspectable.

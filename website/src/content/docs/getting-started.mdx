...
<Code code={extractRegion(pluginLifecycleDart, 'session-plugin-basic')} lang="dart" />
...
4. **Register competing services.**
   ...
   registers at `Priority.elevated`. `CasualPlugin` takes the default (`Priority.normal`). When the host resolves
...
- **Lower `FormalPlugin`'s priority below the default** (try `priority: Priority.low` or `Priority.lowest`) and re-run. Output flips back to `Hello, world.` Same plugins, different winner.
- **Soft-disable `FormalPlugin` via config.** Pass `RuntimeSettings` to the runtime and mark the formal plugin disabled.
 Both plugins still ship, both classes still exist, but the disabled one never wins:
  ...
  <Code code={extractRegion(pluginLifecycleDart, 'getting-started-disable-plugin')} lang="dart" />

...
description: Add Plugin Kit, write a chat plugin and an experimental variant for the same slot, and watch the runtime pick the winner.
---
...
- **Soft-disable `ActivityAwarePlugin` via config.** Pass `RuntimeSettings` to the runtime and mark the experimental plugin disabled.
 Both plugins still ship, both classes still exist, but the disabled one never wins:
  ...
  <Code code={extractRegion(pluginLifecycleDart, 'system-prompt-disable')} lang="dart" />

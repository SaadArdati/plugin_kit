...
Resolution starts with a map lookup, then walks the priority list until it finds the first enabled registration. Singletons cache their instance the moment the registry constructs them; lazy singletons cache on first resolve. Factory registrations call the constructor every time, which is the explicit contract of a factory. Priority-sorted insertion still happens at registration time, not at resolve.
...
`updateSessionSettings` and `updateGlobalSettings` are async. Inside a single call, the runtime reconciles plugins one at a time. Across two concurrent calls, you are responsible for serialization (see the back-to-back toggles entry in [Troubleshooting](/troubleshooting/#two-back-to-back-plugin-toggles-throw-stateerror)).
...
### Can I load plugins from another process, a server, or a sandbox?

Not today. Every plugin compiles into your application binary. The runtime, registry, event bus, and lifecycle hooks all assume in-process calls and share Dart object identity, so a plugin running over IPC, WebSocket, or a wasm boundary cannot participate in resolution, the priority cascade, or the typed event flow as it stands.

Out-of-process plugin loading is **planned for a future release**. It needs real work: a serialization protocol that survives the typed envelope cascade, lifecycle isolation that does not let a remote plugin wedge the host, version negotiation between independently-released remote plugins, and a security model. This release does not attempt any of that. If you need cross-process plugins today, this runtime is not yet what you want, but it is being built toward it.

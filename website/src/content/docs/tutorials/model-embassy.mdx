...
`example/model_embassy/` is a tutorial that teaches Plugin Kit through a model-routing metaphor. An agent presents a `ModelPassport` (model family, requirements). The embassy registers one `VisaOffice` per provider. The first office that can honour the passport issues a `ModelVisa` carrying a ready-to-use client. Offices that cannot serve the request return `null` so the next candidate gets a turn.

If [the villain_lair examples](/tutorials/villain-lair/) cover the runtime concept by concept, this one is a focused study of a single shape: priority-based provider routing for an LLM-style application.
...
```dart
final passport = ModelPassport(family: 'anthropic', id: 'claude-sonnet');

final visa = await session.request<AgentBoardingCall, ModelVisa?>(
  AgentBoardingCall(passport: passport),
);

await visa!.client.complete('Hello!');
```
...
Pin('visa_office_plugin', ['visa_office']): ServiceSettings(
  config: {'api_key': prodKey, 'base_url': 'https://api.prod...'},
),

// Session C: wildcard key targets whichever plugin owns the slot.
Pin.wildcard(['visa_office']): ServiceSettings(
  config: {'api_key': stagingKey},
),
```
...
| If you are... | Read |
|---|---|
| New to provider-routing | 01, 03 |
| Designing a `StatefulPluginService` for routing | 02, 04 |
| Building a multi-tenant runtime | 05 |
| Looking for the working template | 06 |

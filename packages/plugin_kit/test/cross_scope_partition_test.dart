...
void main() {
  group('cross-scope service pin handling', () {
    test(
      'a global plugin pin in settings is applied at init and ignored at '
      'createSession (no silent loss of intent)',
      () async {
    ...
    test(
      'a misspelled plugin id in a service pin throws at init '
    ...
    test(
      'a wildcard service pin survives both init and createSession '
      'partitions (matches whatever winner exists in each scope)',
      () async {
        final runtime = PluginRuntime(
          plugins: [_GlobalScopePlugin(), _SessionScopePlugin()],
        );
        ...
        final session = await runtime.createSession(settings: settings);
        final sessionService = session.registry
            .resolve<_SessionConfigService>(const ServiceId('svc'));
        expect(

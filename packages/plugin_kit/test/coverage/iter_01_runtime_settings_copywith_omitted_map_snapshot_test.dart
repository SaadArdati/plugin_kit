...
void main() {
  test('copyWith snapshots omitted services map when plugins are replaced', () {
    ...
    final settings = RuntimeSettings(
      plugins: {const PluginId('alpha'): const PluginConfig(enabled: true)},
      services: sourceServices,
    );

    final copy = settings.copyWith(
      plugins: {const PluginId('beta'): const PluginConfig(enabled: false)},
    );

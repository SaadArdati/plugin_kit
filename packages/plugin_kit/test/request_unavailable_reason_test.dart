...
void main() {
  group('RequestUnavailableException.reason as enum', () {
    test(
      'noRegistration: thrown when no handler exists for the type pair',
      () async {
        final bus = EventBus();
    ...
    test('noMatchingHandler: thrown when the type pair has handlers but the '
        'priority-merged list is empty for this identifier', () async {
      final bus = EventBus();
    ...
    test('allConceded: bucket lookups by (Request, Response) do not collapse '
        'across nullable / non-nullable Response generics', () async {
    ...
    test('maybeRequest / maybeRequestSync still convert every unavailability '
        'reason to null', () async {
      final bus = EventBus();

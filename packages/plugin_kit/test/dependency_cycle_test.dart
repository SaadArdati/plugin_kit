...
void main() {
  group('dependency cycle detection', () {
    ...
    test('a single-node self-cycle (P depends on P) emits a severe log', () {
      final records = _captureLogs(() {
      ...
      final cycleLogs = records
          ...
          .where(
            (m) =>
                m.toLowerCase().contains('cycle') && m.contains('self_cycle'),
          )
          .toList();
      expect(cycleLogs, isNotEmpty);
    });

    test('a non-cyclic dependency graph (B depends on A, A has no deps) does '
      ...
      expect(
        cycleLogs,
        isEmpty,
        reason: 'must not flag cycles on acyclic graphs; saw: $cycleLogs',
      );
    });

    test('cyclic plugins still attach (severe log is informational, not a '
        'rejection)', () {
      // Suppress noise: we only care about attachment outcome here.
      final runtime = PluginRuntime(plugins: [_CyclicA(), _CyclicB()])..init();
    ...
    test('a three-node cycle (A <- B <- C <- A) emits a severe log naming all '
        'three participants', () {
      final records = _captureLogs(() {

...
- `PluginService.injectSettings` runs during attach. In a stub, call it manually if your service reads `config`. It updates `settings`/`settingsHash`/`config` then fires `onSettingsInjected()`.
- `ctx.bus.on(...)` subscriptions do not auto-cancel. Within a single test the bus is discarded, so this is fine. In production code, use the helpers (anti-patterns.md #6).

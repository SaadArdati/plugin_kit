...
11. `PluginId` values starting with `__pk_` are reserved for internal sentinels (`PluginId.wildcard == '__pk_wildcard__'`, `PluginId.winnerScoped == '__pk_winner__'`; `PluginId` declares `implements String` so the typed identifier compares directly to a String literal). `PluginRuntime.addPlugin` rejects any user-supplied id with that prefix. Pick any other naming; plugin ids conventionally read as lowercase_snake_case (`chat`, `model_router`).

12. Request/response no-answer outcomes are typed exceptions. `context.bus.request<R, S>(req)` and `requestSync<R, S>(req)` throw a `NoRequestAnswerException` subtype: `RequestNotWiredException` (no handler for the type pair, or none matched the identifier; carries `wasIdentifierMismatch` to distinguish) or `AllConcededException` (every handler ran and returned null on a non-nullable `Response`; suggests `maybeRequest`). `maybeRequest` and `maybeRequestSync` are canonical: catch `NoRequestAnswerException` and convert to null. Handler-thrown exceptions propagate through both methods unchanged; `maybeRequest` never swallows them. Reach for `request` only when at least one handler is guaranteed to claim.
